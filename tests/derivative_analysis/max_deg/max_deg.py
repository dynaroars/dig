...
def create_composite(ls, degree, input):
    ...
    for i in range(1,degree):
        comb = comb + list(combinations(variables, i+1)) + list(combinations(const_vars, i+1))
    # if degree>=2:
    #     for c in vars:
    #         for i in range(degree):
    ...
    for tup in comb:
        ...
        temp_list = [1]*len(traces[0]) #the traces must have same length. So, we create a list with all ones same length as a trace length
        for j in tup:
            ...
            for k in range(num_traces):
                ...
                if (not vars[i] in const_vars and not vars[j] in const_vars and is_valid_pair(vars[i],vars[j], const_vars)):
                    ...
                    if len(coeffs)<len(pairs):
                        all_coeffs_consts.append((remove_trailing_zeros(coeffs), const_vals, const_vars))
                        var_composite_deg = max(vars[j][1],(vars[i][1])*degree)
                        ...
                        if len(coeffs)<len(pairs):
                            all_coeffs_consts.append((remove_trailing_zeros(coeffs), const_vals, const_vars))
                            var_composite_deg = max((vars[j][1])*degree,vars[i][1])
                            degs.append(var_composite_deg)
                            # print(func)
            ...
            if len(all_coeffs_consts) >0:
                ...
                for n in range(len(coeff_traces)):
                    for p in range(len(const_traces)):
                        (a,b) = remove_duplicates(const_traces[p], coeff_traces[n])
                        pairs = list(map(lambda x, y: (x, y),a, b))
                        func =  R.lagrange_polynomial(pairs)
                        degree_cons = func.degree()
                        coeffs_re = func.coefficients()
                        const_composite_deg = const_vars_in_all[p][1]*degree_cons
    ...
    while True:
        ...
        if not is_same_index or counter == size:
            ...
            for i in trace_lists:
                arg.append(i)
            traces_list.append(create_composite(arg, degree_of_composite, input))
