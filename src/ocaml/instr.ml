...
class print_type_visitor vtrace mainQ = object(self)
  ...
  method vfunc f =
    if (f.svar.vname =  mainQ )|| (is_vtrace f.svar.vname vtrace) then (
      let s = L.map (fun vi ->
...
let () = begin
    ...
    let trace_src = Sys.argv.(3) in (*instrument for execution*)
    ...
    (* Just print out type info*)
    ignore(visitCilFileSameGlobals
             (new print_type_visitor vtrace mainQ) ast_civl);
    
    
    (* CIVL Instrumentation *)    
    let mainQ_fd:fundec = CM.find_fun ast_civl mainQ in
    ignore(visitCilFileSameGlobals
             (new add_symstates_visitor vtrace) ast_civl);    
