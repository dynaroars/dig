...
class Ideone
{
	static int getchar(final int str, final int idx, final int siz) {
		assert 0 <= idx && idx < siz && 0 <= str & str <= siz;
		return idx == str ? 0 : ' ';
	}
	static int setchar(final int str, final int idx, final int siz, final int c) {
		assert 0 <= idx && idx < siz && 0 <= str & str <= siz;
		if (c == 0) {
	...
	static void strncopy(final int n, final int siz, final int srcsiz, final int destsiz){
		assert (1 <= siz);
		assert (0 <= n);
		assert (n <= siz-1);
	...
	static int mylen(final int k, final int str, final int siz){
